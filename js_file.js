...
form.addEventListener("submit", (e) => {
  ...
  const status = bookInfo.get("book_status") !== null ? "read" : "unread";
...
Book.prototype.toggleBookStatus = function () {
  ...
  this.status = stateTransition[this.status];
...
function displayBook(book) {
  ...
  bookCardClone.querySelector(".status-toggle-btn").textContent =
    book.status.charAt(0).toUpperCase() + book.status.slice(1);
  bookCardClone.querySelector(".status-toggle-btn").classList.add(book.status);
